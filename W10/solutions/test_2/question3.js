...
const range = (num, shouldSkipZero, shouldReverse) => {
    const array = [];
    const firstIndex = shouldSkipZero ? 1  : 0;
    const lastIndex = shouldSkipZero ? num  : num - 1;
    
    if (num === 0 && !shouldSkipZero) {
        console.log([0]);
        return [0];
    }
    ...
    const result = shouldReverse ?  array.reverse() : array;
...
range(3, true, false);
range(0, true, false); // []
range(0, false, true); // [0]
range(0, false, false); // [0]